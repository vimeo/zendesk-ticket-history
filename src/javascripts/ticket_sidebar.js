...
class TicketSidebar {
  ...
  attachEvents() {
    $('.ticket-link').click((e) => {
      e.preventDefault();
      this.client.invoke('routeTo', 'ticket', $(e.currentTarget).data('ticket-id'));
    });
  ...
  formatAssigneeName(name) {
    ...
    first_initial = parts[1].substring(0, 1);

    return parts[0] + ' ' + first_initial + '.';
  } 
